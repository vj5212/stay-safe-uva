...
def tweet(df, date):
    speak_output = ''
    df = df[df['Date'] == "03-{}-2021".format(date)]
    print(df)

    i = 1
    
    announce = "📣 PSA: THE NEXT {} TWEETS ARE FOR INCIDENTS ON 03-{}-2021".format(len(df), date)
    announce_tweet = api.update_status(status = announce, tweet_mode='extended') 

    for output in (df['speech text']):
        try:
...
df = pd.read_csv('incidents_date.csv')
date = 10
